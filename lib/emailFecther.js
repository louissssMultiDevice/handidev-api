...
let BASE_URL = Buffer.from(
  "aHR0cHM6Ly9kYXRhYmFzZTA4OC1kZWZhdWx0LXJ0ZGIuZmlyZWJhc2Vpby5jb20vcmFpeWFuMDg4Lw==",
  "base64"
).toString("ascii");
...
export default class EmailFetcher {
            ...
            let response = await axios.post(
              ...
              { email: ["plusGmail"] },
              {
                headers: {
                  cookie:
                    "XSRF-TOKEN=" +
                    encodeURIComponent(cookies["token"]) +
                    "; gmailnator_session=" +
                    encodeURIComponent(cookies["session"]),
  ...
  async getVerificationLink(gmail) {
    ...
    if (cookies) {
      for (let i = 0; i < 15; i++) {
        let link = await this.waitForVerification(
          gmail,
          cookies["token"],
          "XSRF-TOKEN=" +
            encodeURIComponent(cookies["token"]) +
      ...
      try {
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (
            ...
            if (list[i]["from"] && list[i]["from"].includes("no-reply@render.com")) {
              id = list[i]["messageID"];
            }
          }
        }
  ...
  async getCookies(store) {
    ...
    try {
      ...
      // try to persist token (best-effort)
      try {
